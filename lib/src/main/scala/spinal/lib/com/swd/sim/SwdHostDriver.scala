package spinal.lib.com.swd.sim
...
class SwdHostDriver(cd: ClockDomain, swdioI: Bool, swdioO: Bool, swdioOe: Bool) {
  /** Drive the target side of an SWDIO TriState (read = host -> target). */
  def this(cd: ClockDomain, swdio: TriState[Bool]) = this(cd, swdio.read, swdio.write, swdio.writeEnable)
